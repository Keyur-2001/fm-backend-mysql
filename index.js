...
wss.on("connection", (ws, req) => {
  ...
  if (salesRFQId && userId) {
    ...
    ws.send(
      JSON.stringify({ message: `Connected to SalesRFQ ${salesRFQId} updates` })
    );
  } else {
...
async function startServer() {
  try {
    ...
    const routes = [
      ...
      ["api/sendPurchaseOrder/send-purchase-order", sendPurchaseOrderRoutes],
    ];
    ...
    routes.forEach(([path, route]) => {
      ...
      // import api from './../Fleet_Monkey_Frontend/src/utils/api';
