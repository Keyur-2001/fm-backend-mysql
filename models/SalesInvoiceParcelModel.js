...
class SalesInvoiceParcelModel {
  ...
  static async createSalesInvoiceParcel(data) {
    try {
      ...
      if (response.Status !== 'SUCCESS') {
        throw new Error(response.Message || 'Failed to create Sales Invoice Parcel');
      }

      return { salesInvoiceParcelId: response.SalesInvoiceParcelID, message: response.Message };
    } catch (err) {
      throw new Error(`Database error: ${err.message}`);
    }
  }
  ...
  static async updateSalesInvoiceParcel(id, data) {
    try {
      ...
      const response = result[0][0];
      if (response.Status !== 'SUCCESS') {
        throw new Error(response.Message || 'Failed to update Sales Invoice Parcel');
      }
  ...
  static async deleteSalesInvoiceParcel(id, deletedById) {
    try {
      ...
      const [result] = await pool.query(
        'CALL SP_ManageSalesInvoiceParcelDEV(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        ['DELETE', id, null, null, null, null, null, null, null, null, null, null, deletedById]
      );

      const response = result[0][0];
      if (response.Status !== 'SUCCESS') {
        throw new Error(response.Message || 'Failed to delete Sales Invoice Parcel');
      }
