...
class PInvoiceParcelController {
  ...
  static async getPInvoiceParcelById(req, res) {
    try {
      const { id } = req.params;
      const pInvoiceParcel = await PInvoiceParcelModel.getPInvoiceParcelById(
        parseInt(id)
      );
      if (!pInvoiceParcel) {
  ...
  static async updatePInvoiceParcel(req, res) {
    try {
      ...
      const result = await PInvoiceParcelModel.updatePInvoiceParcel(
        parseInt(id),
        data
      );
      res.status(200).json({
  ...
  static async deletePInvoiceParcel(req, res) {
    try {
      ...
      const result = await PInvoiceParcelModel.deletePInvoiceParcel(
        parseInt(id),
        deletedById
      );
      res.status(200).json({
  ...
  static async getParcelsByPInvoiceId(req, res) {
    try {
      ...
      const parcels = await PInvoiceParcelModel.getParcelsByPInvoiceId(
        parseInt(pInvoiceId)
      );
      res.status(200).json({
        success: true,
