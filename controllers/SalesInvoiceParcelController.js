...
class SalesInvoiceParcelController {
      ...
      res.status(200).json({
        ...
        pagination: {
          ...
          totalPages: Math.ceil(
            result.totalRecords / (parseInt(pageSize) || 10)
          ),
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "SalesInvoiceID, ItemID, ItemQuantity, Rate, and Amount are required.",
          data: null,
        });
      }

      const result = await SalesInvoiceParcelModel.createSalesInvoiceParcel({
        ...data,
        UserID: userId,
      });
  ...
  static async updateSalesInvoiceParcel(req, res) {
    try {
      ...
      const result = await SalesInvoiceParcelModel.updateSalesInvoiceParcel(
  ...
  static async deleteSalesInvoiceParcel(req, res) {
    try {
      ...
      const result = await SalesInvoiceParcelModel.deleteSalesInvoiceParcel(
        salesInvoiceParcelId,
        userId
      );
