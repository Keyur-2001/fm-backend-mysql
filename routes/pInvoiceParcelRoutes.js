...
router.get(
  "/pinvoice/:pInvoiceId",
  PInvoiceParcelController.getParcelsByPInvoiceId
);
...
router.post(
  "/:id/upload",
  authMiddleware,
  upload.single("parcelFile"),
  PInvoiceParcelController.uploadParcelFile
);
...
router.patch("/:id/update-quantity", authMiddleware, async (req, res) => {
  try {
    ...
    const result = await PInvoiceParcelModel.updateParcelQuantity(
      parseInt(id),
      itemQuantity,
      rate,
      userId
...
router.patch("/:id/update-rate", authMiddleware, async (req, res) => {
  try {
    ...
    const PInvoiceParcelModel = require("../models/pInvoiceParcelModel");
    const result = await PInvoiceParcelModel.updateParcelRate(
      parseInt(id),
      rate,
      itemQuantity,
      userId
...
router.patch("/:id/update-certification", authMiddleware, async (req, res) => {
  try {
    ...
    const PInvoiceParcelModel = require("../models/pInvoiceParcelModel");
    const result = await PInvoiceParcelModel.updateParcelCertification(
      parseInt(id),
      certificationId,
      userId
    );
...
router.patch("/:id/update-country", authMiddleware, async (req, res) => {
  try {
    ...
    const PInvoiceParcelModel = require("../models/pInvoiceParcelModel");
    const result = await PInvoiceParcelModel.updateParcelCountryOfOrigin(
      parseInt(id),
      countryOfOriginId,
      userId
    );
